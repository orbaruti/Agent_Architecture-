...
export const workflows: Workflow[] = [
  {
    ...
    steps: [
      ...
      { id: 19, from: 'cto', to: 'qa', action: 'Assigns validation', description: 'CTO assigns QA Engineer to run test plans, regression testing, and release validation.', type: 'delegate' },
      { id: 20, from: 'qa', to: 'qa', action: 'Runs validation', description: 'QA Engineer executes test plans, regression tests, and validates acceptance criteria before release.', type: 'execute' },
      { id: 21, from: 'qa', to: 'cto', action: 'Validation complete', description: 'QA Engineer reports validation results and confirms the feature meets release quality gates.', type: 'respond' },
      { id: 22, from: 'cto', to: 'ceo', action: 'Delivery summary', description: 'CTO provides final delivery summary to CEO. Feature is complete and QA-validated.', type: 'report' },
      { id: 23, from: 'ceo', to: 'board', action: 'Final update', description: 'CEO delivers final update to the Board. Feature has been delivered successfully.', type: 'report' },
    ],
  },
  {
    id: 'bug',
    ...
    steps: [
      ...
      { id: 8, from: 'cto', to: 'qa', action: 'Assigns bug verification', description: 'CTO assigns QA Engineer to verify the bug fix and run regression tests.', type: 'delegate' },
      { id: 9, from: 'qa', to: 'qa', action: 'Verifies fix', description: 'QA Engineer verifies the bug is resolved, runs regression tests, and confirms no new issues.', type: 'execute' },
      { id: 10, from: 'qa', to: 'cto', action: 'Verification complete', description: 'QA Engineer confirms the fix passes verification and regression testing.', type: 'respond' },
      { id: 11, from: 'cto', to: 'coo', action: 'Requests workflow review', description: 'CTO notifies COO to review bug workflow health for patterns or routing issues.', type: 'delegate' },
      { id: 12, from: 'coo', to: 'coo', action: 'Reviews bug workflow health', description: 'COO checks routing correctness, recurring bug patterns, business-impact escalation, and closure validation.', type: 'review' },
      { id: 13, from: 'coo', to: 'cto', action: 'Workflow health report', description: 'COO reports whether the bug workflow needs improvement or process changes.', type: 'respond' },
      { id: 14, from: 'cto', to: 'ceo', action: 'Fix verified', description: 'CTO confirms the bug fix is QA-verified and deployed.', type: 'report' },
      { id: 15, from: 'ceo', to: 'board', action: 'Status update', description: 'CEO updates the Board if the bug had business impact.', type: 'report' },
    ],
  },
  {
    id: 'product-planning',
